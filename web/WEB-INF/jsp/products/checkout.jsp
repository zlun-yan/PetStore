        ...
        <c:choose>
            <c:when test="${sessionScope.user.address_num eq 0}">
                <div class="blankslate">
                    ...
                    <button class="btn btn-primary my-3" type="button" id="zlun-js-checkout-address">Add new address</button>
            ...
            Select delivery address
                ...
                <div class="d-flex flex-wrap">
                    <c:forEach var="address" items="${sessionScope.user.addressList}">
                        <c:if test="${sessionScope.user.default_addr_id eq address.id}">
                            <div class="Box hover-grow border-black-fade m-2 p-2 bg-green-light zlun-js-address-box" style="width: 230px" id="address_${address.id}">
                                ...
                                <hr>
                                <div class="f5">${address.district}&nbsp;${address.details}&nbsp;${address.phone}</div>
                            </div>
                            <input type="hidden" id="default_addr" value="${address.id}">
                        </c:if>
                    </c:forEach>

                    <c:forEach var="address" items="${sessionScope.user.addressList}">
                        <c:if test="${sessionScope.user.default_addr_id ne address.id}">
                            <div class="Box hover-grow border-black-fade m-2 p-2 bg-green-light zlun-hover-box zlun-js-address-box" style="width: 230px" id="address_${address.id}">
                                <div class="f4 text-bold">${address.city} ${address.province}</div>
                                <div class="f4 text-bold">Receiver: ${address.name}</div>
                                <hr>
                                <div class="f5">${address.district}&nbsp;${address.details}&nbsp;${address.phone}</div>
                            </div>
        ...
        <c:forEach items="${requestScope.cartList}" var="cart">
            <div class="border d-flex" style="height: 96px">
                ...
                <div class="p-3 flex-self-center">
                    <span id="num_${cart.id}" class="zlun-checkout-num">${cart.num}</span>
                </div>
                <div class="p-3 flex-self-center">
    ...
    <div id="checkBar">
        <div class="border d-flex mx-3" style="height: 68px;background-color: #E5E5E5">
            ...
            <div class="p-3 flex-self-center">
                <c:choose>
                    <c:when test="${sessionScope.user.address_num eq 0}">
                        <button class="btn btn-sm btn-primary" type="button" id="confirm" disabled="disabled">Confirm</button>
                    </c:when>
                    <c:otherwise>
                        <button class="btn btn-sm btn-primary" type="button" id="confirm">Confirm</button>
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
...
<script src="static/js/products/checkout.js"></script>
